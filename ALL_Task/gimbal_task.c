...
void gimbal_task_func(void const * argument) {
    ...
    static uint8_t last_sw_state = RC_SW_N;  // 遥控器档位开关 上一帧状态 - 档位防抖
    ...
    while (1) {
        ...
        /**************************************** 【最高优先级】遥控器掉线全局急停保护 ****************************************/
        // 遥控器超时判定：超过200ms未收到遥控器数据，判定为遥控器掉线/失联
        ...
        // ===================== 遥控器在线 正常工作逻辑 =====================
            ...
            // 云台模式切换条件：遥控器自定义左按键 或 G键 按下 (仅在非失能模式下生效)
